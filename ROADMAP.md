...
| 4g | 🟡 **Technical Hub (widget H24 espandibile)** — prototipo isolato pronto (`technical-hub.html`), NON ancora integrato in `index.html`. Widget compatto con pad argento spazzolato e orologio reale che, al click, apre un overlay con sfocatura progressiva sullo sfondo e una vista tecnica del vano ascensore: hotspot trasparenti sovrapposti all'immagine, ognuno apre una scheda con lo stato del componente e l'azione di contatto mirata. Nato da un render Gemini fornito dall'utente. Da decidere se integrarlo e dove (vedi nodi aperti sotto) | Sonnet | Sì | Alto |
